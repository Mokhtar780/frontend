...
const Product: React.FC = () => {
  ...
  useEffect(() => {
    let intervalId = null; // تعيين intervalId إلى null

    if (loading) {
        // استدعاء الدالة مرة واحدة عند بدء التحميل
        getDataApi();

        // إعداد interval لتحديث كل 2 ثانية
        intervalId = setInterval(() => {
            getDataApi();
        }, 2000);
    }

    // تنظيف interval عند إلغاء تحميل
    return () => {
        if (intervalId) {
  ...
  return (
    <>
       ...
       (loading) ? (
        ...
        <div className="spinner-border text-primary m-50" role="status">
